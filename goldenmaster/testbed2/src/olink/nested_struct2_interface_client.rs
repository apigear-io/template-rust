...
impl Default for NestedStruct2InterfaceOlinkClient {
    fn default() -> Self {
        Self { data: RwLock::new(NestedStruct2InterfaceData::default()), node: RwLock::new(None), publisher: NestedStruct2InterfacePublisher::default() }
    }
}

impl NestedStruct2InterfaceTrait for NestedStruct2InterfaceOlinkClient {
    fn func1(
        &self,
        ...
        Box::pin(async move {
            if let Some(node) = node {
                ...
                node.invoke_remote(
                    &Name::create_member_id("testbed2.NestedStruct2Interface", "func1"),
                    args,
    ...
    fn func2(
        ...
        Box::pin(async move {
            if let Some(node) = node {
                let (tx, rx) = std::sync::mpsc::sync_channel(1);
                node.invoke_remote(
                    &Name::create_member_id("testbed2.NestedStruct2Interface", "func2"),
                    args,
                    Some(Box::new(move |value| {
                        let _ = tx.send(value);
                    })),
    ...
    fn set_prop1(
        ...
        if let Some(node) = self.node.read().as_ref() {
            node.set_remote_property(&Name::create_member_id("testbed2.NestedStruct2Interface", "prop1"), json!(prop1));
        }
    }
    ...
    fn set_prop2(
        ...
        if let Some(node) = self.node.read().as_ref() {
            node.set_remote_property(&Name::create_member_id("testbed2.NestedStruct2Interface", "prop2"), json!(prop2));
        }
    }
...
impl ObjectSink for NestedStruct2InterfaceOlinkClient {
    ...
    fn olink_on_init(
        ...
        if let Ok(data) = serde_json::from_value::<NestedStruct2InterfaceData>(props) {
