...
/// NATS service adapter for NestedStruct3Interface.
...
pub struct NestedStruct3InterfaceNatsService {
    impl_: Arc<dyn NestedStruct3InterfaceTrait>,
    client: async_nats::Client,
}

impl NestedStruct3InterfaceNatsService {
    pub fn new(
        impl_: Arc<dyn NestedStruct3InterfaceTrait>,
        client: async_nats::Client,
    ) -> Self {
    ...
    fn handle_set_property(
        ...
        match member {
            "prop1" => {
                if let Ok(v) = serde_json::from_slice::<NestedStruct1>(&msg.payload) {
                    self.impl_.set_prop1(&v);
                }
            }
            "prop2" => {
                if let Ok(v) = serde_json::from_slice::<NestedStruct2>(&msg.payload) {
                    self.impl_.set_prop2(&v);
                }
            }
            "prop3" => {
                if let Ok(v) = serde_json::from_slice::<NestedStruct3>(&msg.payload) {
