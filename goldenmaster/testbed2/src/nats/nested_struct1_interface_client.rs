...
const TOPIC_PREFIX: &str = "testbed2.NestedStruct1Interface";
...
/// NATS client adapter for NestedStruct1Interface.
...
pub struct NestedStruct1InterfaceNatsClient {
...
impl NestedStruct1InterfaceNatsClient {
    pub fn new(client: async_nats::Client) -> Self {
        Self { data: RwLock::new(NestedStruct1InterfaceData::default()), client, client_id: new_client_id(), publisher: NestedStruct1InterfacePublisher::default() }
    }
    ...
    fn handle_state(
        ...
        if let Ok(data) = serde_json::from_slice::<NestedStruct1InterfaceData>(&msg.payload) {
            *self.data.write() = data;
        }
    }
}

impl NestedStruct1InterfaceTrait for NestedStruct1InterfaceNatsClient {
    fn func_no_return_value(
        ...
        Box::pin(async move {
            ...
            match client.request(format!("{TOPIC_PREFIX}.rpc.funcNoReturnValue"), payload.into()).await {
                Ok(reply) => {
    ...
    fn func_no_params(&self) -> ApiFuture<'_, Result<NestedStruct1, ApiError>> {
        ...
        Box::pin(async move {
            let payload = serde_json::to_vec(&args).unwrap_or_default();
            match client.request(format!("{TOPIC_PREFIX}.rpc.funcNoParams"), payload.into()).await {
                Ok(reply) => {
                    let value: Value = serde_json::from_slice(&reply.payload).unwrap_or_default();
                    Ok(serde_json::from_value(value).unwrap_or_default())
                }
