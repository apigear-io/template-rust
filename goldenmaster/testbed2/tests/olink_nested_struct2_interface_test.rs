...
use testbed2::olink::nested_struct2_interface_client::NestedStruct2InterfaceOlinkClient;
use testbed2::olink::nested_struct2_interface_service::NestedStruct2InterfaceOlinkService;
...
fn setup_nested_struct2_interface() -> (Arc<NestedStruct2InterfaceOlinkClient>, Arc<NestedStruct2Interface>, olink_common::OlinkLoopback) {
    let impl_ = Arc::new(NestedStruct2Interface::default());
    let service: Arc<dyn objectlink_core::traits::ObjectSource> = Arc::new(NestedStruct2InterfaceOlinkService::new(impl_.clone() as Arc<dyn NestedStruct2InterfaceTrait>));
    let loopback = setup_olink_loopback(service);

    let client = Arc::new(NestedStruct2InterfaceOlinkClient::default());
...
mod tests {
    ...
    async fn test_olink_func1() {
        let (client, _impl, _loopback) = setup_nested_struct2_interface();
    ...
    fn test_olink_set_prop1() {
        let (client, impl_, _loopback) = setup_nested_struct2_interface();
    ...
    fn test_olink_prop1_notify() {
        let (client, _impl, loopback) = setup_nested_struct2_interface();
        ...
        loopback.remote_node.notify_property_change("testbed2.NestedStruct2Interface/prop1", test_value);
    ...
    fn test_olink_prop2_notify() {
        ...
        let test_value = serde_json::to_value(&expected).unwrap();
        loopback.remote_node.notify_property_change("testbed2.NestedStruct2Interface/prop2", test_value);
    ...
    fn test_olink_sig1() {
        ...
        loopback.remote_node.notify_signal("testbed2.NestedStruct2Interface/sig1", json!([null]));
    ...
    fn test_olink_sig2() {
        ...
        loopback.remote_node.notify_signal("testbed2.NestedStruct2Interface/sig2", json!([null, null]));
        let received = rx.try_recv().unwrap();
        let _signal_param_0 = received.0.clone();
