...
/// MQTT service adapter for SimpleArrayInterface.
...
/// `sig/<sig>`.
pub struct SimpleArrayInterfaceMqttService {
    impl_: Arc<dyn SimpleArrayInterfaceTrait>,
    client: Arc<AsyncClient>,
}

impl SimpleArrayInterfaceMqttService {
    pub fn new(
        impl_: Arc<dyn SimpleArrayInterfaceTrait>,
        client: Arc<AsyncClient>,
    ) -> Self {
    ...
    /// Publish propReadOnlyString property change over MQTT (retained).
    pub async fn publish_prop_read_only_string_changed(&self) -> Result<(), rumqttc::v5::ClientError> {
        let value = json!(self.impl_.prop_read_only_string());
        let topic = format!("{}/prop/propReadOnlyString", TOPIC_PREFIX);
        let payload = serde_json::to_vec(&value).unwrap_or_default();
        self.client.publish(&topic, QoS::AtLeastOnce, true, payload).await
    }
    ...
    pub async fn publish_current_state(&self) -> Result<(), rumqttc::v5::ClientError> {
        ...
        self.publish_prop_read_only_string_changed().await?;
        Ok(())
    }
}
