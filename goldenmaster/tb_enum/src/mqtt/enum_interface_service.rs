...
/// MQTT service adapter for EnumInterface.
...
/// `sig/<sig>`.
pub struct EnumInterfaceMqttService {
    impl_: Arc<dyn EnumInterfaceTrait>,
    client: Arc<AsyncClient>,
}

impl EnumInterfaceMqttService {
    pub fn new(
        impl_: Arc<dyn EnumInterfaceTrait>,
        client: Arc<AsyncClient>,
    ) -> Self {
    ...
    pub async fn subscribe_topics(&self) -> Result<(), rumqttc::v5::ClientError> {
        self.client.subscribe(format!("{}/rpc/func0", TOPIC_PREFIX), QoS::AtLeastOnce).await?;
        ...
        self.client.subscribe(format!("{}/rpc/func3", TOPIC_PREFIX), QoS::AtLeastOnce).await?;
        self.client.subscribe(format!("{}/set/prop0", TOPIC_PREFIX), QoS::AtLeastOnce).await?;
        ...
        self.client.subscribe(format!("{}/set/prop3", TOPIC_PREFIX), QoS::AtLeastOnce).await?;
        Ok(())
    }
    ...
    /// Publish prop0 property change over MQTT (retained).
    pub async fn publish_prop0_changed(&self) -> Result<(), rumqttc::v5::ClientError> {
        let value = json!(self.impl_.prop0());
    ...
    pub async fn publish_prop3_changed(&self) -> Result<(), rumqttc::v5::ClientError> {
        let value = json!(self.impl_.prop3());
        let topic = format!("{}/prop/prop3", TOPIC_PREFIX);
        let payload = serde_json::to_vec(&value).unwrap_or_default();
        self.client.publish(&topic, QoS::AtLeastOnce, true, payload).await
    }
    pub async fn publish_sig0(
        &self,
        param0: Enum0Enum,
    ) -> Result<(), rumqttc::v5::ClientError> {
        let args = json!([param0]);
        let topic = format!("{}/sig/sig0", TOPIC_PREFIX);
    ...
    pub async fn publish_current_state(&self) -> Result<(), rumqttc::v5::ClientError> {
        self.publish_prop0_changed().await?;
        self.publish_prop1_changed().await?;
        self.publish_prop2_changed().await?;
        self.publish_prop3_changed().await?;
        Ok(())
    }
}
