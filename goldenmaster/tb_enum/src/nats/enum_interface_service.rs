...
/// NATS service adapter for EnumInterface.
...
pub struct EnumInterfaceNatsService {
    impl_: Arc<dyn EnumInterfaceTrait>,
    client: async_nats::Client,
}

impl EnumInterfaceNatsService {
    pub fn new(
        impl_: Arc<dyn EnumInterfaceTrait>,
        client: async_nats::Client,
    ) -> Self {
    ...
    async fn handle_operation(
        ...
        let result = match member {
            "func0" => {
                ...
                match self.impl_.func0(param_0).await {
                    Ok(value) => json!(value),
                    _ => json!(null),
                }
            }
            ...
            "func2" => {
                ...
                match self.impl_.func2(param_0).await {
                    Ok(value) => json!(value),
                    _ => json!(null),
                }
            }
            "func3" => {
                ...
                match self.impl_.func3(param_0).await {
                    Ok(value) => json!(value),
                    _ => json!(null),
                }
            }
    ...
    fn handle_set_property(
        ...
        match member {
            "prop0" => {
                if let Ok(v) = serde_json::from_slice::<Enum0Enum>(&msg.payload) {
            ...
            "prop2" => {
                if let Ok(v) = serde_json::from_slice::<Enum2Enum>(&msg.payload) {
                    self.impl_.set_prop2(v);
                }
            }
            "prop3" => {
                if let Ok(v) = serde_json::from_slice::<Enum3Enum>(&msg.payload) {
