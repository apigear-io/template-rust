...
use tb_same2::olink::same_struct2_interface_client::SameStruct2InterfaceOlinkClient;
use tb_same2::olink::same_struct2_interface_service::SameStruct2InterfaceOlinkService;
...
fn setup_same_struct2_interface() -> (Arc<SameStruct2InterfaceOlinkClient>, Arc<SameStruct2Interface>, olink_common::OlinkLoopback) {
    let impl_ = Arc::new(SameStruct2Interface::default());
    let service: Arc<dyn objectlink_core::traits::ObjectSource> = Arc::new(SameStruct2InterfaceOlinkService::new(impl_.clone() as Arc<dyn SameStruct2InterfaceTrait>));
    let loopback = setup_olink_loopback(service);

    let client = Arc::new(SameStruct2InterfaceOlinkClient::default());
...
mod tests {
    ...
    async fn test_olink_func1() {
        let (client, _impl, _loopback) = setup_same_struct2_interface();
    ...
    fn test_olink_set_prop1() {
        let (client, impl_, _loopback) = setup_same_struct2_interface();
    ...
    fn test_olink_prop1_notify() {
        let (client, _impl, loopback) = setup_same_struct2_interface();
        let rx = client.publisher().prop1_changed.subscribe();
        let expected: Struct2 = Default::default();
        let test_value = serde_json::to_value(&expected).unwrap();
        loopback.remote_node.notify_property_change("tb.same2.SameStruct2Interface/prop1", test_value);
    ...
    fn test_olink_prop2_notify() {
        ...
        let expected: Struct2 = Default::default();
        let test_value = serde_json::to_value(&expected).unwrap();
        loopback.remote_node.notify_property_change("tb.same2.SameStruct2Interface/prop2", test_value);
    ...
    fn test_olink_sig1() {
        ...
        loopback.remote_node.notify_signal("tb.same2.SameStruct2Interface/sig1", json!([null]));
    ...
    fn test_olink_sig2() {
        ...
        loopback.remote_node.notify_signal("tb.same2.SameStruct2Interface/sig2", json!([null, null]));
        let received = rx.try_recv().unwrap();
        let _signal_param_0 = received.0.clone();
