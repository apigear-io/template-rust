...
use tb_same2::nats::same_enum1_interface_client::SameEnum1InterfaceNatsClient;
use tb_same2::nats::same_enum1_interface_service::SameEnum1InterfaceNatsService;
use std::sync::Arc;
...
async fn test_nats_same_enum1_interface_roundtrip() {
    let impl_ = Arc::new(SameEnum1Interface::default());
    let service = Arc::new(SameEnum1InterfaceNatsService::new(impl_.clone() as Arc<dyn SameEnum1InterfaceTrait>, nats_common::connect().await));
    let _service_sub = service.subscribe();

    let client = Arc::new(SameEnum1InterfaceNatsClient::new(nats_common::connect().await));
