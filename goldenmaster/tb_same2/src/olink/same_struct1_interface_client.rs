...
impl SameStruct1InterfaceTrait for SameStruct1InterfaceOlinkClient {
    fn func1(
        ...
        Box::pin(async move {
            if let Some(node) = node {
                ...
                node.invoke_remote(
                    &Name::create_member_id("tb.same2.SameStruct1Interface", "func1"),
                    args,
    ...
    fn set_prop1(
        ...
        if let Some(node) = self.node.read().as_ref() {
            node.set_remote_property(&Name::create_member_id("tb.same2.SameStruct1Interface", "prop1"), json!(prop1));
        }
    }
