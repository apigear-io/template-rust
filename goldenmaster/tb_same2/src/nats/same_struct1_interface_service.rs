...
/// NATS service adapter for SameStruct1Interface.
...
pub struct SameStruct1InterfaceNatsService {
    impl_: Arc<dyn SameStruct1InterfaceTrait>,
    client: async_nats::Client,
}

impl SameStruct1InterfaceNatsService {
    pub fn new(
        impl_: Arc<dyn SameStruct1InterfaceTrait>,
        client: async_nats::Client,
    ) -> Self {
    ...
    fn handle_set_property(
        ...
        match member {
            "prop1" => {
                if let Ok(v) = serde_json::from_slice::<Struct1>(&msg.payload) {
