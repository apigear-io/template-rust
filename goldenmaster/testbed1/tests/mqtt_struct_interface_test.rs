...
use testbed1::mqtt::struct_interface_client::StructInterfaceMqttClient;
use testbed1::mqtt::struct_interface_service::StructInterfaceMqttService;
use std::sync::Arc;
...
async fn test_mqtt_struct_interface_roundtrip() {
    let impl_ = Arc::new(StructInterface::default());
    let (service_client, service_loop) = mqtt_common::connect("svc-testbed1-struct_interface");
    let service = Arc::new(StructInterfaceMqttService::new(impl_.clone() as Arc<dyn StructInterfaceTrait>, service_client));
    ...
    let (client_client, client_loop) = mqtt_common::connect("cli-testbed1-struct_interface");
    let client = Arc::new(StructInterfaceMqttClient::new(client_client, "cli-testbed1-struct_interface"));
