...
/// NATS service adapter for StructArrayInterface.
...
pub struct StructArrayInterfaceNatsService {
    impl_: Arc<dyn StructArrayInterfaceTrait>,
    client: async_nats::Client,
}

impl StructArrayInterfaceNatsService {
    pub fn new(
        impl_: Arc<dyn StructArrayInterfaceTrait>,
        client: async_nats::Client,
    ) -> Self {
    ...
    async fn handle_operation(
        ...
        let result = match member {
            ...
            "funcEnum" => {
                ...
                match self.impl_.func_enum(param_0.as_slice()).await {
                    Ok(value) => json!(value),
                    _ => json!(null),
                }
            }
    ...
    fn handle_set_property(
        ...
        match member {
            "propBool" => {
                if let Ok(v) = serde_json::from_slice::<Vec<StructBool>>(&msg.payload) {
                    self.impl_.set_prop_bool(&v);
                }
            }
            "propInt" => {
                if let Ok(v) = serde_json::from_slice::<Vec<StructInt>>(&msg.payload) {
                    self.impl_.set_prop_int(&v);
                }
            }
            "propFloat" => {
                if let Ok(v) = serde_json::from_slice::<Vec<StructFloat>>(&msg.payload) {
                    self.impl_.set_prop_float(&v);
                }
            }
            "propString" => {
                if let Ok(v) = serde_json::from_slice::<Vec<StructString>>(&msg.payload) {
                    self.impl_.set_prop_string(&v);
                }
            }
            "propEnum" => {
                if let Ok(v) = serde_json::from_slice::<Vec<Enum0Enum>>(&msg.payload) {
