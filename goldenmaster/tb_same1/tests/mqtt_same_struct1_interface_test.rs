...
use tb_same1::mqtt::same_struct1_interface_client::SameStruct1InterfaceMqttClient;
use tb_same1::mqtt::same_struct1_interface_service::SameStruct1InterfaceMqttService;
use std::sync::Arc;
...
async fn test_mqtt_same_struct1_interface_roundtrip() {
    let impl_ = Arc::new(SameStruct1Interface::default());
    let (service_client, service_loop) = mqtt_common::connect("svc-tb_same1-same_struct1_interface");
    let service = Arc::new(SameStruct1InterfaceMqttService::new(impl_.clone() as Arc<dyn SameStruct1InterfaceTrait>, service_client));
    ...
    let (client_client, client_loop) = mqtt_common::connect("cli-tb_same1-same_struct1_interface");
    let client = Arc::new(SameStruct1InterfaceMqttClient::new(client_client, "cli-tb_same1-same_struct1_interface"));
