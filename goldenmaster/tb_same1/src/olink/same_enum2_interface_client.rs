...
impl Default for SameEnum2InterfaceOlinkClient {
    fn default() -> Self {
        Self { data: RwLock::new(SameEnum2InterfaceData::default()), node: RwLock::new(None), publisher: SameEnum2InterfacePublisher::default() }
    }
}

impl SameEnum2InterfaceTrait for SameEnum2InterfaceOlinkClient {
    fn func1(
        &self,
        param1: Enum1Enum,
        ...
        Box::pin(async move {
            if let Some(node) = node {
                ...
                node.invoke_remote(
                    &Name::create_member_id("tb.same1.SameEnum2Interface", "func1"),
                    args,
    ...
    fn func2(
        ...
        Box::pin(async move {
            if let Some(node) = node {
                let (tx, rx) = std::sync::mpsc::sync_channel(1);
                node.invoke_remote(
                    &Name::create_member_id("tb.same1.SameEnum2Interface", "func2"),
                    args,
                    Some(Box::new(move |value| {
                        let _ = tx.send(value);
                    })),
    ...
    fn set_prop1(
        ...
        if let Some(node) = self.node.read().as_ref() {
            node.set_remote_property(&Name::create_member_id("tb.same1.SameEnum2Interface", "prop1"), json!(prop1));
        }
    }
    ...
    fn set_prop2(
        ...
        if let Some(node) = self.node.read().as_ref() {
            node.set_remote_property(&Name::create_member_id("tb.same1.SameEnum2Interface", "prop2"), json!(prop2));
        }
    }
...
impl ObjectSink for SameEnum2InterfaceOlinkClient {
    ...
    fn olink_on_init(
        ...
        if let Ok(data) = serde_json::from_value::<SameEnum2InterfaceData>(props) {
